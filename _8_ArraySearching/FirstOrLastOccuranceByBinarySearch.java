...
public class FirstOrLastOccuranceByBinarySearch {
  ...
  public static void main(String[] args) {
    try (Scanner sc = new Scanner(System.in)) {
      ...
      System.out.print("\nEnter 'true' to find first occurance and 'false' to find the last occurance : ");
      boolean isFirst = sc.nextBoolean();

      int index = firstOrLastOccuranceByBinarySearch(arr, target, isFirst);
      if (index >= 0 && isFirst == true) {
        System.out.printf("\nElement's first occurance is at index '%d'", index);
      } else if (index >= 0 && isFirst == false) {
        System.out.printf("\nElement's last occurance is at index '%d'", index);
      } else {
